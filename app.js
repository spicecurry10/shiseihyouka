const STORAGE_KEY = "postureEvaluationRecords";
const LEGACY_STORAGE_KEY = "bodyEvalMvpRecords";
...
function addManualPoint(side, event) {
  ...
  target.points = manualPointsToMap(target, metric);
  target.source = "manual";
  state.latestResult = null;
  redraw(target);
  updateStatus();
...
function manualPointsToMap(target, metric) {
  const points = target.manualPoints;
  const map = {};
  ...
  map.__scale = { width: target.canvas.width || 900, height: target.canvas.height || 680 };
  return map;
}
...
function undoPoint(side) {
  ...
  target.points = manualPointsToMap(target, METRICS[els.themeSelect.value]);
...
function clearAllDeviceData() {
  ...
  els.privacyConfirm.checked = false;
  state.latestResult = null;
  renderHistory();
  updateStatus();
  updateResultPrompt();
